...
### Run only during daylight +- 60 min, except if force mode set
[ "$FORCE" ] || check_daylight 60
...
if [ $(bool "$METER") -eq 1 ]; then
    ...
    if [ $(calc "$VALUE == $last" 0) -eq 1 ]; then
        log 1 Skip, value equal last value
        [ "$FORCE" ] || exit
    fi
...
mail="mail $MAILOPTS -a 'From: $EMAIL' -s Sunny-Mail $MAILTO"
...
cat $TMPFILE | mail $MAILOPTS -a "From: $EMAIL" -s Sunny-Mail $MAILTO
